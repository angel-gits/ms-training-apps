...
class TrainingRepository extends ServiceEntityRepository
{
    ...
    */
    ...
    public function findLikeMathcesByValue($value) {
        return $this->createQueryBuilder('t')
            ->where('t.id LIKE :trainingId')
            ->orWhere('t.startDateTime LIKE :startDT')
            ->orWhere('t.status LIKE :tStatus')
            ->setParameter('trainingId', '%' . $value . '%')
            ->setParameter('startDT', '%' . $value . '%')
            ->setParameter('tStatus', '%' . $value . '%')
            ->getQuery()
            ->execute();
    }
}
